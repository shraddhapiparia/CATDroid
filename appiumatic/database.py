...
class Database:
    ...
    def create_tables(self):
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS event_info (event_hash text, suite_id text, frequency integer, termination integer, reward real, qvalue real)") # qvalue column added
    ...
    def create_suite(self):
        ...
        #logger.info("Test generation started at {}.".format(str(creation_time)))  # TODO: change to human-readable time
    ...
    # Added by KK
    def get_event_qvalues(self, event_hashes, suite_id): # kk
        cursor = self.db_connection.cursor()
        placeholders = ','.join('?' for _ in event_hashes)
        query = "SELECT event_hash, qvalue FROM event_info " \
                "WHERE event_hash IN ({}) AND suite_id=?".format(placeholders)
        cursor.execute(query, list(event_hashes) + [suite_id])

        event_qvalues = OrderedDict()
        rows = cursor.fetchall()
        for row in rows:
            event_hash = row[0]
            event_value = row[1]
            event_qvalues[event_hash] = event_value

        for event_hash in event_hashes:
            if event_hash not in event_qvalues:
                event_qvalues[event_hash] = 0

        return event_qvalues


    def update_event_qvalue(self, suite_id, event_hash, new_value): # kk
        logger.debug("Updating event qvalue for {}.".format(event_hash))
        cursor = self.db_connection.cursor()
        event_query = "SELECT qvalue FROM event_info WHERE event_hash=? AND suite_id=?"
        cursor.execute(event_query, (event_hash, suite_id))

        rows = cursor.fetchall()
        if rows:
            update_query = "UPDATE event_info SET qvalue=? WHERE event_hash=? AND suite_id=?"
            cursor.execute(update_query, (new_value, event_hash, suite_id))
        else:
            insert_query = "INSERT INTO event_info VALUES (?, ?, ?, ?, ?, ?)"
            cursor.execute(insert_query, (event_hash, suite_id, 0, 0, 0.0, new_value))
